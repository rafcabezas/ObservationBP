...
public struct ObservationTracking: Sendable {
    ...
    struct Entry: @unchecked Sendable {
        ...
        func addWillSetObserver(_ changed: @Sendable @escaping () -> Void) -> Int {
            context.registerTracking(for: properties, willSet: changed)
        }

        func addDidSetObserver(_ changed: @Sendable @escaping () -> Void) -> Int {
            context.registerTracking(for: properties, didSet: changed)
        }
...
private func generateAccessList<T>(_ apply: () -> T) -> (T, ObservationTracking._AccessList?) {
